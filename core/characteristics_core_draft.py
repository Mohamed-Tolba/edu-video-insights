...
To do:
    1. Getting video duration: I am using the function that extracts the duration
       from youtube. It is more accurate to use opencv to find the duration of a downloaded video.
       I have built the function that uses opencv, but have not included it in the dependent functions; 
       such as word per minute, scene chaneg freq, etc. functions.
    2. The word count function(s) rely on getting the transcript from youtube, we may need to
       develop a function that extract the transcript from a downloaded video (e.g., using NLP).
...
class CharacsExtractor:
    ...
    def extract_transcript(self, video_path):
        ...
        return result                       # Return the extracted transcript
    ...
    def extract_video_duration_sec(self, video_path): # Uses OpenCV package. It counts the frames and divides the count by the fps
...
if __name__ == "__main__":
    ...
    video_duration_min = CharacsExtractor_obj.get_video_duration_min(video_id)
    video_speaking_rate = CharacsExtractor_obj.extract_word_per_minute(video_id)
    video_scene_change_rate = CharacsExtractor_obj.extract_scene_change_freq(video_id, video_path, 30)
    
    print(f"video url: {video_url}")
    print(f"video duration min: {video_duration_min}")
    print(f"video speaking rate: {video_speaking_rate}")
    print(f"video scene change rate: {video_scene_change_rate}")
    ...
    CharacsExtractor_obj.delete_file(video_path)
     ...
     # Replace with an actual video ID for testing
